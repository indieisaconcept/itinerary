...
helper = function (/* Object */ helpers, /* Object */ manifest) {
    ...
    return function (/* String */ route, /* String */ item) {
    ...
    return function (/* String */ route) {
        ...
        while ( label = key.pop() ) {
            ...
            if (value) {
                ...
                temp = {
                    global  : util._.isBoolean(value.global) ? value.global : true,
                    config  : value.config   || {},
        ...
        util._.find(config, function(item) {

            var isVertical  = item.template.vertical,
                allowGlobal = util._.isBoolean(item.global) ? item.global : true;

            if (isVertical && allowGlobal) {
        ...
        result = config.reduce(function (previous, current) {
             ...
             current  = helpers(route, current);

             if (current.root || current.global) {
                delete current.root;
