...
var processor = function (/* Object */ manifest, /* Object */ options) {
    ...
    return function (/* String */ route) {
        ...
        while ( label = key.pop() ) {
            ...
            // config.length === 0 [ current route ]
            // key.length    === 0 [ root route    ]

            if (value) {
                if (config.length === 0 || key.length === 0) {
                   config.unshift({
                        config  : value.config   || {},
        ...
        result = config.reduce(function (previous, current) {
            ...
            if (current) {
                 ...
                 Object.keys(helpers).forEach(function (key) {
                    var value = util._.deepGet(current.config, key);
                    if (value) {
                        util._.deepSet(current.config, key, helpers[key](route, value, current.template, manifest));
                    }
                 });
