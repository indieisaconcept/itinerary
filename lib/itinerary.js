...
retrieve = function (key) {
    ...
    return !key && this || util._.deepGet(this.config, key);
...
inherits = function (config, key, current) {
    ...
    var cnd, temp, verts, citem, inherit, allow = false, currConfig = current.config || {};
    ...
    temp = {
	inherit : !cnd.root && isBool(inherit) ? inherit : true,
	config  : currConfig,
	template: current.template || currConfig.template || {}
    };
    ...
    if (cnd.root) {
        ...
        if (verts[0]) {
            ...
            temp.inherit = allow ? 'global' : 'none';
        } else {
            citem = config[config.length - 1] || {};
            temp.inherit = citem.inherit ? 'all' : 'root';
        }
...
processor = function (/* Object */ manifest, /* Object */ options) {
    ...
    return function (/* String */ route, /* Function */ callback) {
        ...
        key = ['route'].concat(key).filter(function (item) {
